...
# Usage:  bash nids_ml/artifacts/article_1/run_article_mutation.sh [--dry-run]
# ────────────────────────────────────────────────────────────────────────────
set -uo pipefail
...
a = json.loads(Path("../sip-dataset/mutation_holdout_split/attack/test.json").read_text())["dataset"]
b = json.loads(Path("../sip-dataset/mutation_holdout_split/benign/test.json").read_text())["dataset"]
recs = a + b
